...
pub fn standardize_value_with_remainder_i128(
    ...
    let remainder = match value.unsigned_abs().checked_rem_euclid(step_size) {
        Some(r) => r.cast::<i128>(env).safe_mul(value.signum(), env),
        None => {
    ...
    //     .ok_or_else(|| panic_with_error!(env, ErrorCode::MathError))?
    //     .cast::<i128>(env)
    //     .safe_mul(value.signum(), env);

    let standardized_value = value.safe_sub(remainder, env);

    (standardized_value, remainder)
}

pub fn get_proportion_i128(env: &Env, value: i128, numerator: u128, denominator: u128) -> i128 {
...
pub fn get_proportion_u128(env: &Env, value: u128, numerator: u128, denominator: u128) -> u128 {
    ...
    //     value.cast::<u128>(env)?
    // }
    numerator > denominator / 2 && denominator > numerator {
...
pub fn on_the_hour_update(env: &Env, now: i64, last_update_ts: i64, update_period: i64) -> i64 {
