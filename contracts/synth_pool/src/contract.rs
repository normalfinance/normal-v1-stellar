...
use crate::{
    controller::{ self, position },
    events::PoolEvents,
    math::{
        self,
        bit_math::checked_mul_shift_right,
        liquidity_math::convert_to_liquidity_delta,
        tick_math::{ tick_index_from_sqrt_price, MAX_SQRT_PRICE_X64, MIN_SQRT_PRICE_X64 },
    },
    pool::PoolTrait,
    position::{ get_position_by_ts, get_position_info, Position, PositionUpdate },
    reward::{ calculate_collect_reward, get_reward_by_token, RewardInfo },
    storage::{
        get_pool,
        save_pool,
        utils::{ self, is_initialized, set_initialized },
        Pool,
        PoolParams,
    },
    tick::Tick,
...
use normal::{
    constants::{ DAY_IN_SECONDS, INSTANCE_BUMP_AMOUNT, INSTANCE_LIFETIME_THRESHOLD }, error::{ ErrorCode, NormalResult }, math::{casting::Cast, safe_math::SafeMath}, oracle::get_oracle_price, utils::{ convert_i128_to_u128, is_approx_ratio }, validate_bps, validate_int_parameters
};
...
impl PoolTrait for SynthPool {
    ...
    fn initialize(
        ...
        let tick_current_index = tick_index_from_sqrt_price(&params.initial_sqrt_price);

        let tick_arrays = Map::new(&env).set(
            tick_current_index,
            TickArray::new(&env, env.current_contract_address(), tick_current_index, params.tick_spacing)
        );
        let tick_steps: i32 = 10;

        for i in 0..tick_steps {
            let upper_tick_index = params.tick_spacing.safe_mul(i, &env)?;
            let lower_tick_index = params.tick_spacing.safe_mul(-i, &env)?;
        ...
        // PoolEvents::initialize(&env, from, amount);
    }
    ...
    fn initialize_tick_array(env: Env, sender: Address, start_tick_index: i32) {
        ...
        pool.initiliaze_tick_array(&env, start_tick_index);
    }
    ...
    fn create_position(env: Env, sender: Address, tick_lower_index: i32, tick_upper_index: i32) {
        ...
        let mut position = get_position_info(&env, &sender);
        // posi
    ...
    fn close_position(env: Env, sender: Address, position_ts: u64) {
        ...
        token_contract::Client::new(&env, &pool.share_token).burn(&sender, 1);
    }
    ...
    fn increase_liquidity(
        ...
        let tick_array_lower = match pool.tick_arrays.get(tick_array_lower_index) {
            Some(ta) => ta,
            None => {
                return Err(ErrorCode::AdminNotSet);
            }
        };
        let tick_array_upper = match pool.tick_arrays.get(tick_array_upper_index) {
            Some(ta) => ta,
            None => {
                return Err(ErrorCode::AdminNotSet);
            }
        ...
        utils::mint_shares(&env, &pool.share_token, &env.current_contract_address(), 1);

        PoolEvents::increase_liquidity(&env, &sender, pool.token_a, pool.token_b, delta_a, delta_b);
    }
    ...
    //     // PoolEvents::collect_fees(&env, to, amount);
    // }
    ...
    //     // PoolEvents::remove_liquidity(&env, to, out_a, out_b);
    // }
    ...
    //     // PoolEvents::swap(&env, to, buy_a, out, in_max);
    // }
