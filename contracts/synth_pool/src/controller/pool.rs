...
pub fn update_pool_price(env: &Env, pool: &Pool) -> NormalResult<()> {
    ...
    let delta_b = controller::liquidity::calculate_collateral_liquidity_token_delta(
    ...
    let token_collateral_client = token_contract::Client::new(&env, &pool.token_b);

    if liquidity_delta > 0 {
        // pull collateral from market
        token_collateral_client.transfer_from(
        ...
        pool.transfer_b(&env.current_contract_address(), &pool.market, &delta_b);
    }
