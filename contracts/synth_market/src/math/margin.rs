...
//         .safe_div(
//             SPOT_IMF_PRECISION_U128.safe_add(
//                 size_sqrt.safe_mul(imf_factor.cast()?)?.safe_div(100_000)?
//             )?
//         )?
//         .cast::<u32>()?;
...
//         market_position.worst_case_liability_value(oracle_price_data.price, market.contract_type)?;
...
//         0
//     } else {
//         let margin_ratio = user_custom_margin_ratio.max(
...
//         worse_case_liability_value.safe_mul(margin_ratio.cast()?)?.safe_div(MARGIN_PRECISION_U128)?
//     };
...
//         .safe_add(
//             market_position.margin_requirement_for_lp_shares(
//                 market.amm.order_step_size,
//                 valuation_price
//             )?
//         )?;

//     let unrealized_asset_weight = market.get_unrealized_asset_weight(
...
//     let open_order_margin_requirement = if
//         track_open_order_fraction &&
//         worst_case_base_asset_amount != 0
//     {
...
pub fn calculate_max_withdrawable_amount(market_index: u16, user: &User) -> DriftResult<u64> {
    ...
    let token_amount = user.get_spot_position(market_index)?.get_token_amount(spot_market)?;
