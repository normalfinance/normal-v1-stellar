...
use crate::{
    ...
    state::liquidity_position::{LiquidityPosition, LiquidityPositionUpdate},
};
...
pub fn next_position_modify_liquidity_update(
...
) -> NormalResult<LiquidityPositionUpdate> {
    ...
    let growth_delta_synthetic = fee_growth_inside_a.wrapping_sub(position.fee_growth_checkpoint_a);
    let fee_delta_synthetic =
        checked_mul_shift_right(position.liquidity, growth_delta_synthetic).unwrap_or(0);

    let growth_delta_quote = fee_growth_inside_b.wrapping_sub(position.fee_growth_checkpoint_b);
    let fee_delta_quote =
        checked_mul_shift_right(position.liquidity, growth_delta_quote).unwrap_or(0);
